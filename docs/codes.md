...
| `output.exists` | A path the package would create already exists, in any form — file, directory, symbolic link or a dangling link. Nothing is ever overwritten, and the whole extraction is refused before anything is written. | `entry` | `a_target_file_that_already_exists_refuses_the_whole_extraction`, `a_leaf_target_that_is_a_pre_existing_symlink_is_refused` |
