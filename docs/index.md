...
| [codes.md](codes.md) | Every stable `archive.*`, `extract.*`, `input.*`, `metadata.*` and `output.*` code — 102 of them — with its category, meaning, the numeric fields its error carries, the exit status it classifies to, and the test that asserts it; kept in step with the sources by `scripts/check-codes.py`. |
