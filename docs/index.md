...
| [codes.md](codes.md) | Every stable `archive.*` and `metadata.*` code with its category, meaning, the numeric fields its error carries, and the test that asserts it; kept in step with the sources by `scripts/check-codes.py`. |
...
| [testing.md](testing.md) | Test layout file by file, how to run the suite and the coverage gate, what the truncation and mutation sweeps guarantee, the fixture policy, the planned fuzz targets, later compatibility testing, and the private-corpus pointer. |
...
| CLI contract: a command, flag, JSON field, or exit status | [architecture.md](architecture.md#command-contract-and-json-envelope) and [Exit statuses](architecture.md#exit-statuses), [README.md](../README.md) quick start, [roadmap.md](roadmap.md) if it closes a milestone, [CHANGELOG.md](../CHANGELOG.md) |
