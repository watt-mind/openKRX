...
| [codes.md](codes.md) | Every stable `archive.*`, `create.*`, `extract.*`, `input.*`, `manifest.*`, `metadata.*`, `output.*` and `repack.*` code — 147 of them — with its category, meaning, the numeric fields its error carries, the exit status it classifies to, and the test that asserts it; kept in step with the sources by `scripts/check-codes.py`. |
...
| [testing.md](testing.md) | Test layout file by file, how to run the suite and the coverage gate, what the truncation and mutation sweeps guarantee, the fixture policy, the five fuzz targets with their bounded per-push lane and what it does not prove, the retained-regression replay and the weekly campaign over a cumulative corpus, the informational public-API compatibility report and why it is not a gate, later compatibility testing, and the private-corpus pointer. |
| [limits-measured.md](limits-measured.md) | One machine's measurement of what the published limits cost: for each of six limits, a synthetic package exactly at it and one a step past it, driven through `inspect`, `validate-structure` and `extract`, with the peak resident memory, the wall time and the stable code each over-limit package was reported with — written by `scripts/measure-limits.py`, and a measurement rather than a guarantee. |
| [work-packages.md](work-packages.md) | The dependency-ordered implementation specifications KRX-01 to KRX-08, each with owned paths, acceptance criteria, verification and the unknowns it must not resolve by assumption. |
| [schema/openkrx-envelope.v1.schema.json](schema/openkrx-envelope.v1.schema.json) | The machine-readable form of the `--json` envelope for `schema_version` 1: the failure shape, each command's success `data`, the stable-code pattern and the diagnostic categories, as one JSON Schema (draft 2020-12) that `scripts/check-schema.py` validates every JSON golden against. |
...
| [SECURITY.md](../SECURITY.md) | Current surface, how to report a vulnerability, the threat model package support must satisfy, and the threat-model mappings for the command-line input, archive, metadata, extraction planning, extraction output, creation and repacking layers, each with its residual risks. |
...
| [crates/openkrx-cli/skills/openkrx/SKILL.md](../crates/openkrx-cli/skills/openkrx/SKILL.md) | Agent skill, embedded in the binary and written out by `openkrx skill`: the rules that always apply, the conformance and cryptography boundary, the JSON envelope and every exit status, the capabilities-to-extract, create and repack workflow, and the reporting rules an AI agent follows when using the CLI on a package. |
| [fuzz/README.md](../fuzz/README.md) | The separate `cargo-fuzz` package: why it is not a workspace member, the five targets with their entry points and the two invariants, how to run them and reproduce an artifact, and the committed lockfile rule. |
| [tests/fixtures/README.md](../tests/fixtures/README.md) | Why almost no fixture is committed, what the golden fixtures are and how they are generated, and what a future one would have to record. |
...
| Limits: a default value, a new limit, or what a limit is enforced against | [architecture.md](architecture.md#limits) (the limit table and its rationale), [codes.md](codes.md) if a code is added, `crates/openkrx-core/examples/limit_packages.rs` and [limits-measured.md](limits-measured.md) if the measured set changes, [CHANGELOG.md](../CHANGELOG.md) |
...
| CLI contract: a command, flag, JSON field, or exit status | [architecture.md](architecture.md#command-contract-and-json-envelope) and [Exit statuses](architecture.md#exit-statuses), [codes.md](codes.md) if a code is added or its status changes, [README.md](../README.md) quick start, [the agent skill](../crates/openkrx-cli/skills/openkrx/SKILL.md), the golden cases under `tests/golden/` for the bytes it changes, [releasing.md](releasing.md#what-ships) if it changes what a release archive carries, [roadmap.md](roadmap.md) if it closes a milestone, [CHANGELOG.md](../CHANGELOG.md) |
| JSON envelope: a field added, removed, renamed or redefined, a command that starts or stops rendering one, or a new stable-code head | [schema/openkrx-envelope.v1.schema.json](schema/openkrx-envelope.v1.schema.json) — the schema moves with the envelope, in the same pull request — [architecture.md](architecture.md#the-published-schema), [testing.md](testing.md#the-envelope-schema) if a gate changes, the golden cases under `tests/golden/` for the bytes it changes, [CHANGELOG.md](../CHANGELOG.md) |
| Tests and fixtures: a new test file, a new sweep, a fuzz target, or any committed fixture | [testing.md](testing.md), [tests/fixtures/README.md](../tests/fixtures/README.md) for a fixture, [fuzz/README.md](../fuzz/README.md) for a target, [conformance.md](conformance.md) if it changes which test holds a rule |
