...
| [codes.md](codes.md) | Every stable `archive.*`, `create.*`, `extract.*`, `input.*`, `manifest.*`, `metadata.*` and `output.*` code — 133 of them — with its category, meaning, the numeric fields its error carries, the exit status it classifies to, and the test that asserts it; kept in step with the sources by `scripts/check-codes.py`. |
...
| [crates/openkrx-cli/skills/openkrx/SKILL.md](../crates/openkrx-cli/skills/openkrx/SKILL.md) | Agent skill, embedded in the binary and written out by `openkrx skill`: the rules that always apply, the conformance and cryptography boundary, the JSON envelope and every exit status, the capabilities-to-extract-and-create workflow, and the reporting rules an AI agent follows when using the CLI on a package. |
