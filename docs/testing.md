...
A few tests need a platform primitive that is not portable. The symbolic link
tests are behind `cfg(unix)`, because Windows cannot create a symbolic link
without developer mode or elevation. The rule they hold is not skipped there:
`mod junctions` in `extract.rs`, `create.rs` and `repack.rs` runs the same
scenarios against a *directory junction*, which is a reparse point `mklink /J`
creates with no privilege at all, so the Windows half of the rule —
