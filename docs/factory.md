...
openKRX supports an explicitly launched Claude master orchestrator.
...
Use the `develop` branch of `watt-mind/openKRX`. From its root:
...
repos:
  - name: openkrx
    path: /absolute/runner/checkout/openKRX
    github: watt-mind/openKRX
...
```sh
factory doctor --repo openkrx
factory queue --repo openkrx
