...
Every failure openKRX reports carries a dotted `archive.*` or `metadata.*`
code, and those codes are part of the public contract. This check extracts
...
SEGMENT = r"[a-z0-9_]+"
SOURCE_CODE = re.compile(rf'"((?:archive|metadata)(?:\.{SEGMENT})+)"')
DOC_CODE = re.compile(rf"`((?:archive|metadata)(?:\.{SEGMENT})+)`")
