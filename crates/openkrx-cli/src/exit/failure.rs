...
pub struct Failure {
    ...
    /// Position in the manifest's `attachments` array, when the failure
    /// concerns one. It is not a central-directory index: for a refused
    /// request, no archive exists.
...
impl Failure {
    ...
    /// A package openKRX wrote that its own reader did not accept.
    #[must_use]
